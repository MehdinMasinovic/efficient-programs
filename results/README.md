...
| v0.3-optimizeio | 2024-01-05 | Mehdin | Memory-mapped I/O optimization | - Replaced buffered I/O with memory mapping (mmap) for direct file access<br>- Added direct memory access for CSV parsing<br>- Optimized string handling with assign()<br>- Pre-allocated vectors based on file size<br>- Disabled C/C++ stream synchronization, untyied cout from cin, and set a 1MB custom output buffer for std::cout to reduce system calls during output operations | - Time: 42.93s<br>- Cycles: 167.6B<br>- IPC: 1.14 |
| v0.4-string_encoding | 2024-01-08 | Simon Josef Kreuzpointner | Encode strings as integers in order to increase comparison performance | - thorougly inspect dataset <br> - map each possible character to a new value between 0 and 36 <br> - create a single number by chaining together the values of the different characters <br> - Since one 64-bit integer is not enough, multiple integers have to be used, which requires the creation of a custom class and logic for handling comparison and equality | - No performance improvement <br> - Time: 903.57 s (timeout) <br> - Cycles: 3 477B <br> - IPC: 1.67 |
| v0.5-string_hashing | 2024-01-08 | Simon Josef Kreuzpointner | Encode strings as integer via hash | - use `std::hash` for strings to perform hashing <br> - Output can be saved in a single 64-bit integer | - Time: 39.86 s <br> - Cycles: 160B <br> - IPC: 1.24 |
